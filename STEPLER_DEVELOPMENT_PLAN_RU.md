...
Stepler - новая версия HotkeyHandler, спроектированная с нуля как стабильная системная утилита для исправления текста, набранного в неверной раскладке, управления горячими клавишами и безопасной работы с активным приложением.

План допускает переиспользование текущего проекта HotkeyHandler, но не закрепляет его архитектурные ограничения.

## 1. Цели проекта

- Стабильно исправлять текст по `Pause` и `ScrollLock`.
...
Переиспользование из HotkeyHandler:
...
- unit-test cases из `HotkeyHandler.Tests`.
...
### Этап 0. Инвентаризация HotkeyHandler
...
- меню включения `Pause`, `ScrollLock`, `Caps Lock`, автозапуска;
