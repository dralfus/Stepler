...
После нажатия `ScrollLock` приложение должно исправить ошибочные слова, не повреждая корректную часть строки.
...
### 4.4 Обработка ScrollLock

- При включенной настройке `ScrollLock` должно искать слова, похожие на ввод в неверной раскладке.
...
- Приложение должно писать лог в файл `hotkey_log.txt` рядом с запущенным исполняемым файлом.
- Лог должен содержать:
  ...
  - результат обработки;
  - время операции от получения `Pause`/`ScrollLock` до полного завершения действия приложения;
...
- Время обработки `Pause` и `ScrollLock` должно измеряться монотонным таймером от момента получения hotkey до момента, когда приложение завершило замену/отказ и восстановило служебное состояние.
...
Текущая реализация собирается командой:

```powershell
dotnet build HotkeyHandler.csproj -nologo -c Debug -p:UseAppHost=true -v minimal
```

Это команда для существующего проекта, а не обязательное требование к технологии.

Допускаются другие языки, IDE и системы сборки, если они позволяют реализовать требования приложения и получить запускаемый Windows-артефакт.

Сборка должна создавать запускаемый файл приложения, например `HotkeyHandler.exe` для текущей Windows-реализации.

Для ручного тестирования сборку необходимо выкладывать в:

```text
<рабочая-папка>\bin\<имя-ветки>\
```

Например:

```text
F:\distr\system\HotkeyHandler\bin\develop\
```

После успешной сборки и исправления багов опубликованный `HotkeyHandler.exe` должен быть запущен из этой папки.
...
При тестировании hotkey обязательно читать `hotkey_log.txt`, потому что он показывает активный путь обработки и ошибки буфера обмена.

### 7.3 Минимальный ручной smoke-test

- В Notepad: `k.,jdm` + `Pause` -> `любовь`.
- В Notepad: `k.,jdm` + `ScrollLock` -> исправленное слово без обрезания.
- В Notepad: `вальс поле long ghbdtn vbh` + `ScrollLock` -> корректный префикс сохраняется, ошибочный хвост исправляется.
- В Word: проверить `Pause` и `ScrollLock` на одном слове и на строке из нескольких слов.
- В PowerShell: проверить, что `Pause`/`ScrollLock` не вставляют случайный буфер и не ломают командную строку.
...
- unit-тесты проходят;
- опубликован runnable `HotkeyHandler.exe` в `bin\<имя-ветки>`;
...
- `hotkey_log.txt` не содержит ошибок восстановления буфера после успешных сценариев;
- пользовательский буфер обмена не меняется после обработки `Pause` и `ScrollLock`.
...
- Инструкции для агентной разработки: `AGENTS.md`.
