...
**Status:** implementation complete; ручной smoke с `en-GB` ожидает проверки.
Спецификация: `docs/english_layout_variants_spec_ru.md`.
...
- [x] `en-US` (`0x0409`) продолжает распознаваться.
- [x] `en-GB` (`0x0809`) распознаётся как английская целевая раскладка.
- [x] Другие варианты `en-*` распознаются по primary language bits Windows
  LANGID.
- [x] Русская раскладка (`0x0419`) не выбирается как английская.
- [x] При нескольких английских раскладках выбор детерминирован и следует
  порядку Windows.
- [x] Обычный layout switcher и terminal clipboard fallback используют общий
  resolver.
- [x] При отсутствии `en-*` сохраняется `Unsupported` без выбора другой
  раскладки.
- [x] Добавлены unit-тесты и выполнен полный `cargo test --workspace`.
- [ ] Выполнен ручной smoke с установленной `en-GB` в обычном приложении и
  терминальном fallback.
